...
public class AdhocToXmlTransform {
    ...
    protected void properties(AdhocProperties properties, List<XmlAdhocProperty> xmlAdhocProperties) {
        for (String key : properties.getProperties()
                                    .keySet()) {
            ...
            if (propertyValue != null && !(propertyValue instanceof String)) {
                xmlAdhocProperty.setValueClass(propertyValue.getClass()
                                                            .getName());
            }
    ...
    protected String propertyValueToString(Object value) {
        ...
        throw new AdhocException("Property value type " + value.getClass()
    ...
    protected XmlAdhocReport report(AdhocReport adhocReport) {
        ...
        if (adhocReport.getColumns() != null && !adhocReport.getColumns()
                                                            .isEmpty()) {
            for (AdhocColumn adhocColumn : adhocReport.getColumns()) {
                xmlAdhocReport.getColumn()
                              .add(column(adhocColumn));
            }
        }
        if (adhocReport.getGroups() != null && !adhocReport.getGroups()
                                                           .isEmpty()) {
            for (AdhocGroup adhocGroup : adhocReport.getGroups()) {
                xmlAdhocReport.getGroup()
                              .add(group(adhocGroup));
            }
        }
        if (adhocReport.getSorts() != null && !adhocReport.getSorts()
                                                          .isEmpty()) {
            for (AdhocSort adhocSort : adhocReport.getSorts()) {
                xmlAdhocReport.getSort()
                              .add(sort(adhocSort));
            }
        }
        if (adhocReport.getSubtotals() != null && !adhocReport.getSubtotals()
                                                              .isEmpty()) {
            for (AdhocSubtotal adhocSubtotal : adhocReport.getSubtotals()) {
                xmlAdhocReport.getSubtotal()
                              .add(subtotal(adhocSubtotal));
            }
        }
        if (adhocReport.getComponents() != null && !adhocReport.getComponents()
                                                               .isEmpty()) {
            for (AdhocComponent adhocComponent : adhocReport.getComponents()) {
                xmlAdhocReport.getComponent()
                              .add(component(adhocComponent));
            }
        }
        if (adhocReport.getProperties() != null && !adhocReport.getProperties()
                                                               .isEmpty()) {
    ...
    protected XmlAdhocColumn column(AdhocColumn adhocColumn) {
        ...
        if (adhocColumn.getProperties() != null && !adhocColumn.getProperties()
                                                               .isEmpty()) {
    ...
    protected XmlAdhocGroup group(AdhocGroup adhocGroup) {
        ...
        if (adhocGroup.getProperties() != null && !adhocGroup.getProperties()
                                                             .isEmpty()) {
    ...
    protected String color(Color color) {
        ...
        String hex = Integer.toHexString(rgb & colorMask)
                            .toUpperCase();
    ...
    protected void component(AdhocComponent adhocComponent, XmlAdhocComponent xmlAdhocComponent) {
        ...
        if (adhocComponent.getProperties() != null && !adhocComponent.getProperties()
                                                                     .isEmpty()) {
    ...
    protected XmlAdhocChartSerie chartSerie(AdhocChartSerie adhocChartSerie) {
        ...
        if (adhocChartSerie.getProperties() != null && !adhocChartSerie.getProperties()
                                                                       .isEmpty()) {
    ...
    protected XmlAdhocFilter filter(AdhocFilter adhocFilter) {
        ...
        if (adhocFilter.getRestrictions() != null && !adhocFilter.getRestrictions()
                                                                 .isEmpty()) {
            for (AdhocRestriction adhocRestriction : adhocFilter.getRestrictions()) {
                xmlAdhocFilter.getRestriction()
                              .add(restriction(adhocRestriction));
            }
        }
    ...
    protected void restriction(AdhocRestriction adhocRestriction, XmlAdhocRestriction xmlAdhocRestriction) {
        ...
        if (adhocRestriction.getProperties() != null && !adhocRestriction.getProperties()
                                                                         .isEmpty()) {
    ...
    protected void valueRestriction(AdhocValueRestriction adhocValueRestriction, XmlAdhocValueRestriction xmlAdhocValueRestriction) {
        ...
        if (adhocValueRestriction.getValues() != null && !adhocValueRestriction.getValues()
                                                                               .isEmpty()) {
            for (String value : adhocValueRestriction.getValues()) {
                xmlAdhocValueRestriction.getValue()
                                        .add(value);
            }
        }
    }
