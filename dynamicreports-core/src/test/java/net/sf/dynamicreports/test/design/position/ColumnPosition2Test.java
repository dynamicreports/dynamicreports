...
public class ColumnPosition2Test extends ColumnPosition1Test {
    ...
    protected void testBand(DRDesignBand band) {
        ...
        Assert.assertEquals(2, list1.getComponents()
                                    .size());
        componentPositionTest(list1, 0, 0, 575, 32);
        Assert.assertTrue(list1.getComponents()
                               .get(0) instanceof DRDesignTextField);
        Assert.assertTrue(list1.getComponents()
                               .get(1) instanceof DRDesignList);

        DRDesignList list2 = (DRDesignList) list1.getComponents()
                                                 .get(1);
        Assert.assertEquals(ListType.VERTICAL, list2.getType());
        Assert.assertEquals(2, list2.getComponents()
                                    .size());
        componentPositionTest(list2, 191, 0, 384, 32);
        Assert.assertTrue(list2.getComponents()
                               .get(0) instanceof DRDesignList);
        Assert.assertTrue(list2.getComponents()
                               .get(1) instanceof DRDesignList);

        DRDesignList list3 = (DRDesignList) list2.getComponents()
                                                 .get(0);
        Assert.assertEquals(ListType.HORIZONTAL, list3.getType());
        Assert.assertEquals(2, list3.getComponents()
                                    .size());
        componentPositionTest(list3, 191, 0, 384, 16);
        Assert.assertTrue(list3.getComponents()
                               .get(0) instanceof DRDesignTextField);
        Assert.assertTrue(list3.getComponents()
                               .get(1) instanceof DRDesignTextField);

        DRDesignList list4 = (DRDesignList) list2.getComponents()
                                                 .get(1);
        Assert.assertEquals(ListType.HORIZONTAL, list4.getType());
        Assert.assertEquals(2, list4.getComponents()
                                    .size());
        componentPositionTest(list4, 191, 16, 384, 16);
        Assert.assertTrue(list4.getComponents()
                               .get(0) instanceof DRDesignTextField);
        Assert.assertTrue(list4.getComponents()
                               .get(1) instanceof DRDesignTextField);

        // column1
        componentPositionTest(list1.getComponents()
                                   .get(0), 0, 0, 191, 32);

        // column2
        componentPositionTest(list3.getComponents()
                                   .get(0), 0, 0, 192, 16);
        ...
        // column4
        componentPositionTest(list4.getComponents()
                                   .get(0), 0, 0, 128, 16);
